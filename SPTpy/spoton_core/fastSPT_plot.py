...
####把计算得到的跳跃长度分布可视化，并且可以在同一个图上叠加模型模拟的曲线。
####接收输入数据是HistVecJumps（跳跃长度分布的横坐标，即各个bin的中心位置），还有emp_hist(实验册的的概率密度矩阵，形状为（n_timepoints,n_bins）)
    ...
    for i in range(JumpProb.shape[0] - 1, -1, -1):
        ...
        for j in range(1, JumpProb.shape[1]):  ## Looks like we are manually building an histogram. Why so?
            ...
            plt.fill([x1, x1, x2, x2], [y1, y2, y2, y1],
                     color=colour_element)  # /!\ TODO MW: Should use different colours
