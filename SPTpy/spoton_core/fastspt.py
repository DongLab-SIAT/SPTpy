...
##
## ==== Main functions
##计算在一系列单分子轨迹中，粒子的跳跃长度分布（PDF或者CDF），即粒子在不同时间步长之间的移动距离
...
####这个函数用于拟合跳跃长度分布的模型，采用非线性最小二乘拟合方法，将模型的结果与实验数据对比，以找到最佳拟合参数。
####输出：最佳拟合参数，最小的残差平方和
...
####计算二态模型（BOUND/UNBOUND）下，一个给定时间间隔curr_dT和跳跃距离向量r的理论概率密度（PDF）值
...
####计算三态模型（FAST/MEDIUM/BOUND）下给定参数和时间间隔的理论PDF
...
###给定一组参数猜测parameter_guess(二态或者三态)、实验测得得JumpProb/JumpProbCDF以及模型设置，批量调用上述_compute_2states
###和_compute_3states,生成模型在各个时间点上对应的PDF或CDF，以便与实验数据做最小二乘拟合。
...
####给定最终拟合出来的参数字典fitparams，直接生成模型预测的跳跃长度分布（PDF或归一化版本），用于可视化或后续分析
...
###是一个交替计数（镜像方法）精确求解了有吸收边界条件下的生存概率
###该函数返回一个校正因子h，用于修正自由扩散态或结合态下，粒子在给定初始位置z和扩散系数D，历经时间CurrTime后仍留在板层内的概率累计影响
