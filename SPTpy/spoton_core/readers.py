...
def read_mosaic(fn, framerate, pixelsize):
    return read_arbitrary_csv(fn, col_traj="Trajectory", col_x="x", col_y="y", col_frame="Frame",
                              framerate=framerate / 1000., pixelsize=pixelsize / 1000.)
...
def read_trackmate_csv(fn, framerate):
    ...
    return read_arbitrary_csv(fn, col_traj="TRACK_ID", col_x="POSITION_X", col_y="POSITION_Y", col_frame="FRAME",
                              framerate=framerate / 1000., cb=cb)
...
def read_trackmate_xml(fn):
    ...
    try:
        spaceunit.decode('ascii')  # the only reasonable non-ascii unit is microns.
    except:
        spaceunit = 'um'
    ...
    for particle in x['Tracks']['particle']:
        traces.append([(float(d['@x']), float(d['@y']), float(d['@t']) * framerate, float(d['@t'])) for d in
                       particle['detection']])
    return traces
...
def traces_to_csv(traces):
    ...
    """
    print(
        "WARNING: deprecated use of 'traces_to_csv' in fastspt.readers, use it in 'fastspt.writers.traces_to_csv instead'.")
...
###把任意格式的CSV/TSV文件读入为fastSPT需要的轨迹列表格式
def read_arbitrary_csv(fn, col_x="", col_y="", col_frame="", col_t="t",
    ...
    cols = da.columns
    if (not (col_traj in cols and col_x in cols and col_y in cols and col_frame in cols)) or (
            not (col_t in cols) and framerate == None):
...
####把一个已经准备好的PandasDataFrame，按轨迹ID分组，然后按帧号排序，每个组构造一个轨迹列表[(x,y,t,frame),......]
####输出out是一个python列表，内含若干条轨迹，每条又是一个点元组列表
